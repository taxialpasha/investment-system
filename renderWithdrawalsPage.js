...
function renderWithdrawalsPage() {
      ...
      for (const withdrawal of processedWithdrawals) {
        ...
        html += `
          <tr class="border-b hover:bg-gray-50">
            <td class="p-2 border">${investor ? investor.name : 'غير معروف'}</td>
            <td class="p-2 border font-bold text-red-600">${numberWithCommas(withdrawal.amount)} د.ع</td>
            <td class="p-2 border">${formatDate(withdrawal.processingDate || withdrawal.date)}</td>
            <td class="p-2 border">
  ...
  // الوظائف المفقودة التي تسبب أخطاء
  ...
  function exportWithdrawalsReport() {
    ...
    let csvContent = "data:text/csv;charset=utf-8,\uFEFF"; // إضافة BOM للدعم الصحيح للعربية
...
// عرض نافذة إضافة سحب جديد مع خيار تحديد تاريخ التنفيذ
function showWithdrawalModal(investor = null) {
    ...
    // إضافة خانة جديدة لتحديد تاريخ تنفيذ السحب
    const modalBody = document.querySelector('#withdrawal-modal .modal-body');
    
    // التحقق من وجود الخانة مسبقًا
    if (!document.getElementById('withdrawal-execution-date-container')) {
      // إنشاء خانة جديدة لتاريخ تنفيذ السحب
      const executionDateContainer = document.createElement('div');
      executionDateContainer.id = 'withdrawal-execution-date-container';
      executionDateContainer.className = 'form-group';
      executionDateContainer.innerHTML = `
        <div class="alert alert-info mb-3">
          <i class="fas fa-info-circle"></i>
          <div>
            <p class="font-bold">تاريخ تنفيذ السحب هو التاريخ الذي يتم فيه إعطاء المبلغ للمستثمر.</p>
            <p class="text-sm">يمكنك تحديد تاريخ تنفيذ السحب بشكل منفصل عن تاريخ تسجيل السحب.</p>
          </div>
        </div>
        <label class="form-label">تاريخ تنفيذ السحب</label>
        <input 
          type="date" 
          id="withdrawal-execution-date"
          class="w-full"
          value="${getCurrentDate()}"
        />
        <p class="text-xs text-gray-500 mt-1">تاريخ إعطاء المبلغ للمستثمر</p>
      `;
      
      // إضافة الخانة بعد خانة تاريخ السحب
      const withdrawalDateContainer = Array.from(modalBody.querySelectorAll('.form-group')).find(
        group => group.querySelector('#withdrawal-date')
      );
      
      if (withdrawalDateContainer) {
        withdrawalDateContainer.parentNode.insertBefore(
          executionDateContainer, 
          withdrawalDateContainer.nextSibling
        );
      } else {
        // إذا لم يتم العثور على خانة تاريخ السحب، أضف في نهاية النموذج
        modalBody.appendChild(executionDateContainer);
      }
    } else {
      // إذا كانت الخانة موجودة بالفعل، فقط قم بتحديث القيمة
      document.getElementById('withdrawal-execution-date').value = getCurrentDate();
    }
  ...
  // تعديل وظيفة إضافة سحب للتعامل مع تاريخ التنفيذ
  function addWithdrawal() {
    ...
    const executionDate = document.getElementById('withdrawal-execution-date').value || date;
    ...
    const transaction = {
      ...
      amount,
      date,
      executionDate, // تاريخ تنفيذ السحب (إعطاء المبلغ للمستثمر)
    ...
    if (!isScheduled) {
      ...
      showNotification(`تم تسجيل سحب أموال بمبلغ ${numberWithCommas(amount)} د.ع بنجاح، وسيتم تنفيذه بتاريخ ${formatDate(executionDate)}`, 'success');
    } else {
      showNotification(`تم جدولة سحب أموال بمبلغ ${numberWithCommas(amount)} د.ع بتاريخ ${formatDate(scheduledDate)}, وسيتم تنفيذه بتاريخ ${formatDate(executionDate)}`, 'success');
    }
  ...
  // تعديل وظيفة عرض تفاصيل السحب في صفحة السحوبات
  function renderWithdrawalsPage() {
    const mainContent = document.getElementById('main-content');
          ...
          <table class="w-full border-collapse">
            <thead>
              <tr class="bg-gray-100 text-right">
                ...
                <th class="p-2 border">تاريخ الجدولة</th>
                <th class="p-2 border">تاريخ التنفيذ</th>
                <th class="p-2 border">تاريخ إعطاء المبلغ</th>
                <th class="p-2 border">الملاحظات</th>
                <th class="p-2 border">الإجراءات</th>
              </tr>
            </thead>
      ...
      for (const withdrawal of sortedScheduled) {
        ...
        html += `
          <tr class="border-b hover:bg-gray-50 ${isUrgent ? 'bg-red-50' : ''}">
            ...
            <td class="p-2 border">${formatDate(withdrawal.executionDate || withdrawal.scheduledWithdrawalDate)}</td>
            <td class="p-2 border">${withdrawal.notes || '-'}</td>
            <td class="p-2 border">
              <div class="flex gap-1">
                <button class="process-withdrawal-btn btn-icon btn-sm btn-outline-primary" data-id="${withdrawal.id}" title="تنفيذ الآن">
      ...
      for (const withdrawal of processedWithdrawals) {
        ...
        html += `
          <tr class="border-b hover:bg-gray-50">
            ...
            <td class="p-2 border font-bold text-red-600">${numberWithCommas(withdrawal.amount)} د.ع</td>
            <td class="p-2 border">${formatDate(withdrawal.processingDate || withdrawal.date)}</td>
            <td class="p-2 border">${formatDate(withdrawal.executionDate || withdrawal.processingDate || withdrawal.date)}</td>
            <td class="p-2 border">
              ${withdrawal.isScheduled ? 
                `<span class="badge badge-yellow">مجدول</span>` : 
                `<span class="badge badge-green">فوري</span>`}
